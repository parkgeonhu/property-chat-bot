...
export const isSatisfySC = async (x, y) => {
    ...
    let schoolNumber = 0;
    const schoolType=['초등학교', '중학교', '고등학교']
    ...
    if (isValid) {
        ...
        for (let document of data.documents) {
            ...
            if (schoolType.includes(category_name[2])) {
    ...
    const isSatisfied = schoolNumber >= statisfaction ? true : false;
...
export const isSatisfyMT = async (x, y) => {
    const {keyword, radius, statisfaction} = nearCondtion["대형마트"]
...
export const isSatisfy = async (x,y) => {
    ...
    return {
        ...
        "대형마트" : {
            is_satisfied : keywordSurround[2]
        },
    }
}
